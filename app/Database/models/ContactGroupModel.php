...
class ContactGroupModel {
	...
	public static function get_all_to_custom_select( $type ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT id, title FROM $group_table WHERE type = %s ORDER BY title ASC", array( $type ) ), ARRAY_A );
		return array(
			'data' => $results,
		);
	}
	...
	public static function destroy( $id ) {
		...
		try {
			$wpdb->delete( $group_table, array( 'id' => $id ), array( '%d' ) );
			return true;
		} catch ( \Exception $e ) {
			return false;
		}
	...
	public static function get_groups_count( $type ) {
		global $wpdb;
		$group_table = $wpdb->prefix . ContactGroupSchema::$table_name;
		return absint( $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM $group_table WHERE type = %s", array( $type ) ) ) );
	}
