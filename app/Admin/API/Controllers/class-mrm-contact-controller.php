...
class MRM_Contact_Controller extends MRM_Base_Controller {
    ...
    public function import_contacts_get_attrs( WP_REST_Request $request ) 
    {
        ...
        try{
            ...
            if( isset( $options['headers'] ) && empty( $options['headers'] ) ){
                return $this->get_error_response( __( "Unable to read file", "mrm"), 400 );
            }
            $result = array(
    ...
    public function import_contacts( WP_REST_Request $request ) {
        ...
        try {
            ...
            $csv_contacts = $reader->getRecords();
            ...
            foreach($csv_contacts as $csv_contact) {
                ...
                /**
                 * Looping thriugh each row from csv and create a contact
                 */
                ...
                if ($contact_email && is_email( $contact_email )) {
                    ...
                    $is_exists = MRM_Contact_Model::is_contact_exist( $contact_email );
                    ...
                    $skipped[] = $contact_args;
                }
            }
            $result = array(
                'total'                => $totalCount,
                'skipped'              => count( $skipped ),
                'existing_contacts'    => count( $exists ),
            );
    ...
    public function get_filtered_contacts( WP_REST_Request $request )
    {
        ...
        $contacts = MRM_Contact_Model::get_filtered_contacts( $params['status'], $params ['group_id'], $perPage, $offset, $search );
